...
public:
      ...
      .automatically_declare_parameters_from_overrides(true))
  {
    ...
    if (!has_parameter("RTCM_REPUBLISH_ENABLED")) {
      declare_parameter("RTCM_REPUBLISH_ENABLED", false);
    }
    if (!has_parameter("UBX_RXM_RTCM_ENABLED")) {
      declare_parameter("UBX_RXM_RTCM_ENABLED", false);
    }
    rtcm_republish_enabled_ = get_parameter("RTCM_REPUBLISH_ENABLED").as_bool();
    ubx_rxm_rtcm_enabled_ = get_parameter("UBX_RXM_RTCM_ENABLED").as_bool();
...
private:
  ...
  {
    if (!rtcm_republish_enabled_ || !rtcm_pub_) {
      return;
    }
    std::ostringstream oss;
    for (auto b : f->buf) {
  ...
  {
    if (!ubx_rxm_rtcm_enabled_ || !ubx_rxm_rtcm_pub_) {
      return;
    }
    RCLCPP_DEBUG(
